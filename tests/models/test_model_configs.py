...
Seams tested:
- ``configs/model/inception_next.yaml`` and ``inception_small.yaml`` build
  InceptionNeXtEncoder instances with the expected stage layouts.
- ``configs/model/convgamer_small.yaml`` builds a ConvGamerEncoder instance
  with the expected stage layout and no head.
"""
...
def test_inception_next_config_builds() -> None:
    cfg = _load_cfg("inception_next")
...
def test_jepa_config_composition() -> None:
    """JEPA stack resolves with matching feature dims and Appendix A values."""
    overrides = [
        "model=jepa",
        "data=jepa",
        "trainer=jepa",
